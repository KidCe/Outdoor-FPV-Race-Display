...
BusHub75Matrix::BusHub75Matrix(const BusConfig &bc) : Bus(bc.type, bc.start, bc.autoWhite) {
  ...
  // Paint a complete frame into the driver's inactive DMA buffer, then publish it
  // with flipDMABuffer(). This prevents the scan engine from observing partial rows.
  ...
  // The verified standalone test uses the positive clock phase. Keep that
  // phase fixed for the custom 80x40 panel build; other HUB75 targets retain
  // the existing user-configurable behaviour.
#ifdef WLED_HUB75_P4_80X40
...
#ifdef WLED_HUB75_P4_80X40
      // The panel test identified the plain shift-register profile as the
      // working setting for this 80x40 panel with SM5166P-marked driver ICs.
...
#ifdef WLED_HUB75_P4_80X40
  // Two 80x40 quarter-scan panels use the same logical pixel count as an
  // 80x80 matrix, but the quarter-scan DMA geometry is only 20 pixels high.
  // The generic height check below therefore misses this large framebuffer
  // and leaves it at 8-bit colour depth, exhausting classic ESP32 RAM.
  // Four bits preserve full white output while releasing 25.6 kB of DMA RAM.
...
*/
  ...
  */
  ...
  if (!display->begin() ) {
    ...
    const uint32_t refreshRate = display->calculated_refresh_rate;
    _framePeriodMs = refreshRate ? static_cast<uint16_t>((1000U + refreshRate - 1U) / refreshRate) : 17;
    _lastFrameFlipAt = 0;
    _valid = true;
    ...
    if (_ledsDirtySecondary) d_free(_ledsDirtySecondary); // should not happen
    // AI: end
    ...
    // AI: below section was generated by an AI
    if (_ledBuffer) {
      _ledsDirtySecondary = (byte*) d_malloc(getBitArrayBytes(_len));
      if (_ledsDirtySecondary) setBitArray(_ledsDirtySecondary, _len, false);
    }
    _dmaBufferIndex = 0;
    // AI: end
  }
...
void IRAM_ATTR BusHub75Matrix::setPixelColor(unsigned pix, uint32_t c) {
  ...
  if (_ledBuffer) {
    ...
    if (_ledBuffer[pix] != fastled_col) {
      ...
      // AI: below section was generated by an AI
      if (_ledsDirtySecondary) setBitInArray(_ledsDirtySecondary, pix, true);
      // AI: end
      _frameDirty = true;
    }
  }
    ...
    // AI: below section was generated by an AI
    // The dirty bitmap tracks pending publication, not the color currently
    // held by the inactive DMA buffer. Always paint black so a disappearing
    // glyph is actually cleared before the next atomic buffer flip.
...
void BusHub75Matrix::show(void) {
  ...
  if (_ledBuffer) {
    // AI: below section was generated by an AI
    // Keep the continuously scanned DMA buffer immutable while building the next
    ...
    if (!hasDirtyPixels) return;

    // The dependency does not expose its previous-buffer-free flag. Waiting one
    // measured DMA frame after the last flip keeps the buffer we are about to
    // paint out of the scan engine, including rapid state-triggered updates.
    if (_lastFrameFlipAt) {
      while (millis() - _lastFrameFlipAt < _framePeriodMs) delay(1);
    }

    // Each DMA buffer owns its own dirty bitmap. A logical change is painted
    // once into each inactive buffer, so later flips cannot revive an old race
    // scene while unchanged pixels avoid a complete 80x80 repaint.
    ...
    if (_isVirtual) virtualDisp->flipDMABuffer();
    else            display->flipDMABuffer();
    _lastFrameFlipAt = millis();
    setBitArray(dirty, _len, false);
    ...
    // AI: below section was generated by an AI
    // The low-memory path paints directly into the inactive DMA buffer from
    // setPixelColor(). Publish it only after the caller has completed the frame.
    if (_lastFrameFlipAt) {
      while (millis() - _lastFrameFlipAt < _framePeriodMs) delay(1);
    }
    if (_isVirtual) virtualDisp->flipDMABuffer();
...
void BusHub75Matrix::cleanup() {
  ...
  // AI: below section was generated by an AI
  if (_ledsDirtySecondary != nullptr) d_free(_ledsDirtySecondary); _ledsDirtySecondary = nullptr;
  // AI: end
}
