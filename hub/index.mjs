...
export class TrustedStore {
  ...
  async restore() { if (!this.persistencePath) return null; try { const state = JSON.parse(await readFile(this.persistencePath, 'utf8')); const active = state.activeEvent; const base = state.trustedSnapshot; if (state.version !== 1 || !active || !isObject(active) || !validIdentifier(active.eventSessionId)) return null; if (!base) { this.eventSessionId = active.eventSessionId; this.eventContext = clone(active.event ?? null); this.metadataOverride = Boolean(state.metadataOverride); this.status = { connection: 'joining', source: '', event: this.eventContext?.name ?? '', quality: 'unknown', raceStatus: null }; return null; } if (!isObject(base) || active.eventSessionId !== base.eventSessionId) return null; const result = validateSnapshot(base); if (!result.valid) return null; if (active.event && (active.event.id !== base.event.id || active.event.name !== base.event.name) && !state.metadataOverride) return null; this.eventSessionId = active.eventSessionId; this.eventContext = clone(active.event ?? base.event); this.metadataOverride = Boolean(state.metadataOverride); this.trustedSnapshot = clone(base); this.snapshotSequence = Number.isSafeInteger(state.snapshotSequence) && state.snapshotSequence >= 0 ? state.snapshotSequence : 0; this.sourceRevisions = isObject(state.sourceRevisions) ? new Map(Object.entries(state.sourceRevisions).filter(([key, value]) => validIdentifier(key) && typeof value === 'string')) : new Map(); this.announcementHistory = Array.isArray(state.announcementHistory) ? state.announcementHistory.filter(item => validateAnnouncement(item).valid && item.eventSessionId === this.eventSessionId).map(clone) : []; this.announcements = new Map(this.announcementHistory.filter(item => item.status === 'active').map(item => [item.announcementId, clone(item)])); this.snapshot = staleSnapshot(this.trustedSnapshot, 'persisted_recovery'); if (this.metadataOverride) { this.snapshot.event = clone(this.eventContext); this.trustedSnapshot.event = clone(this.eventContext); } this.#syncAnnouncements(); this.status = { connection: 'reconnecting', source: this.snapshot.sources[0]?.provider ?? '', event: this.snapshot.event.name, quality: 'stale', raceStatus: currentRaceStatus(this.snapshot) }; return clone(this.snapshot); } catch { return null; } }
}
