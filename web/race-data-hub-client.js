...
export class RaceDataHubClient {
  constructor({ hubUrl, fetchImpl = globalThis.fetch, EventSourceImpl = globalThis.EventSource, onState = () => {}, now = () => Date.now(), storage = globalThis.localStorage, storageKey = "fpv-race-hub-trusted-v1" } = {}) {
    this.base = new URL(hubUrl || "http://localhost:4175"); this.fetchImpl = typeof fetchImpl === "function" ? fetchImpl.bind(globalThis) : fetchImpl; this.EventSourceImpl = EventSourceImpl; this.onState = onState; this.now = now; this.storage = storage; this.storageKey = storageKey; this.stream = null; this.retryTimer = 0; this.retryAttempt = 0; this.generation = 0;
