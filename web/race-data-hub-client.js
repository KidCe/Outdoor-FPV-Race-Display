...
function validateStatus(data) {
  object(data, "status data"); noUnknown(data, new Set(["connection", "source", "event", "quality", "message"]), "status data"); if (!CONNECTIONS.has(data.connection)) throw new Error("Hub status connection is invalid."); string(data.source, "status source", { max: 120 }); string(data.event, "status event", { max: 200 }); if (!QUALITY_STATES.has(data.quality)) throw new Error("Hub status quality is invalid."); if (data.message !== undefined) string(data.message, "status message", { max: 500 }); return data;
}
...
export class RaceDataHubClient {
  constructor({ hubUrl, fetchImpl = globalThis.fetch, EventSourceImpl = globalThis.EventSource, onState = () => {}, now = () => Date.now(), storage = globalThis.localStorage, storageKey = "fpv-race-hub-trusted-v1" } = {}) {
    ...
    this.state = { connection: "disabled", snapshot: null, quality: "unknown", lastDataAt: null, sourceCapturedAt: null, hubEpoch: null, eventSessionId: null, streamSequence: 0, snapshotSequence: 0, announcements: [], error: "", needsReset: false };
    try { const cached = JSON.parse(this.storage?.getItem(this.storageKey) || "null"); if (cached?.snapshot) { const recovered = staleSnapshot(validateHubSnapshot(cached.snapshot)); this.state = { ...this.state, snapshot: recovered, quality: "stale", lastDataAt: Number(cached.persistedAt) || null, sourceCapturedAt: recovered.capturedAt, eventSessionId: recovered.eventSessionId, announcements: sortAnnouncements(recovered.activeAnnouncements.filter(item => item.status === "active")), connection: "reconnecting", error: "Recovering the last trusted Hub snapshot." }; } } catch {}
  }
  ...
  apply(envelope) {
    ...
    if (epochChanged) this.setState({ connection: "reconciling", snapshot: null, announcements: [], quality: "unknown", lastDataAt: null, sourceCapturedAt: null, streamSequence: 0, snapshotSequence: 0, eventSessionId: envelope.eventSessionId, needsReset: false, error: "Hub restarted; waiting for a fresh snapshot." });
