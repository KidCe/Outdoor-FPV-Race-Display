...
- `DisplayScene` owns schedule projection, missing-channel recovery, canonical race-status presentation, preset selection, stable WLED schema compilation, state bindings, and preview rendering. Preview and output consume the same scene. Live race text, status, and pilots never participate in the schema hash.
...
5. `OutputSession` sends the selected scene whenever Live output is enabled, suppressing semantically unchanged publications after a completed send. The sent-state marker includes schema identity, bound values, and brightness; disconnect, reconnect, and live-output deactivation invalidate it so a new connection or resumed overlay is repainted. The FPV host always uses a black schema background and does not expose WLED background effects. Connection recovery and schema repair require no operator button sequence.
