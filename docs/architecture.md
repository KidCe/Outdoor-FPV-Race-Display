...
5. `OutputSession` sends the selected scene whenever Live output is enabled, suppressing semantically unchanged publications after a completed send. The sent-state marker includes schema identity, bound values, brightness, and background-effect settings; disconnect, reconnect, and live-output deactivation invalidate it so a new connection or resumed overlay is repainted. Connection recovery and schema repair require no operator button sequence.
